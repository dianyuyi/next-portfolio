...
export function* getSidePageCollectRequestSaga({ payload }: CollectAction) {
  try {
...
function* getSidePageCollectSagas() {
  yield all([takeLatest(actionTypes.GET_PAGE_COLLECT_REQUEST, getSidePageCollectRequestSaga)])
}

export default getSidePageCollectSagas
