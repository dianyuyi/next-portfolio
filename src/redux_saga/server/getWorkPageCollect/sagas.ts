...
export function* getWorkPageCollectRequestSaga({ payload }: CollectAction) {
  try {
...
function* getWorkPageCollectSagas() {
  yield all([takeLatest(actionTypes.GET_PAGE_COLLECT_REQUEST, getWorkPageCollectRequestSaga)])
}

export default getWorkPageCollectSagas
